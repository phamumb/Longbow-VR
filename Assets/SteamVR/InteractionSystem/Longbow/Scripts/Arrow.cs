	...
	public class Arrow : MonoBehaviour
	{
		...
		void OnCollisionEnter( Collision collision )
		{
			if ( inFlight )
			{
                ...
                EnemyHealth hitEnemy = collision.collider.gameObject.GetComponent<EnemyHealth>();

                //Inc shots here
                StatManager.shotAccum += 1;

                if (hitEnemy)
                {
                    //Inc hits counter here and invoke TakeDamage();
                    StatManager.hitAccum += 1;
                    hitEnemy.TakeDamage();
                }
                else
                {
                    StatManager.currStreak = 0;
                }
                ...
                Destroy(this.gameObject); //Destroy if it does not hit anything
			}
		}
